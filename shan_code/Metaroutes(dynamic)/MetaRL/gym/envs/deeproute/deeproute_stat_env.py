...
Default_task = {'topo_file': "topo2.json"}
...
class DeeprouteStatEnv(gym.Env):
    ...
    def sample_tasks(self, num_tasks):
        ...
        topology_files = ["topo2.json", "topo2.json", "topo2.json"]
    ...
    def get_flow_loss_and_delivery_time(self):
        global_flow_loss = self.backend._loss_flows # get global flow loss
        # get flow average flow completion time
        global_average_delivery_time = 0
        for index in range(len(self.backend.nodes)):
            if self.backend._delivered_flows_node[index] > 0:
                temp = self.backend._delivery_time_node[index] / self.backend._delivered_flows_node[index] 
            else:
                temp = 0
            global_average_delivery_time += temp
        global_average_delivery_time = global_average_delivery_time / len(self.backend.nodes)
        return global_flow_loss, global_average_delivery_time
    ...
    def update_estimation(self):
        ...
        for index, node in enumerate(self.backend.nodes):
            ...
            if self.backend._delivered_flows_local[index] > 0:
                temp = -self.backend._delivery_time_local[index] / self.backend._delivered_flows_local[index]
            else:
                temp = 0
    ...
    def get_state(self):
        ...
        # get the destination of current flow
        for node in self.backend.nodes:
            local_ob = []
