...
def loopingFiveMin():
	...
	#old models are in bucket original2018models

	globalbucket = client.get_bucket('original2018models')

	blobs=client.list_blobs('original2018models')
	...
	for e in all_edges:
		print(e['name'])
		edgename=e['name']
		srcname = e['ends'][0]['name']
		destname = e['ends'][1]['name']	
		...
		try:
			...
			createblobname='model-ALBQ--DENV.h5'
			modelblob=globalbucket.blob(createblobname)
			print(modelblob)
			modelblob.download_to_filename(createlocalmodel)
			modelname=createlocalmodel
		...
		tmpdict={}
		tmpdict['src']=srcname
		tmpdict['dest']=destname
		tmpdict['predicted']=pv
		all5minpredictions.append(tmpdict)

	

	print(len(all_edges))
	result5minpredictions={}
	result5minpredictions['timestamped']=time.time()
	result5minpredictions['predictions']=all5minpredictions
	
	print(result5minpredictions)
