...
def main():
	...
	print("scaled data")
	scaled=scaler.fit_transform(training_1yr_dataDF)
	print(scaled)
	...
	#reframed.drop(reframed.iloc[:,35:68],axis=1,inplace=True)
	print(reframed.head())
	...
	#taking 90% of 1 year of trained data
	n_train_hours=364*24 
	...
	#fit network
	history=model.fit(train_X,train_y,epochs=10,batch_size=24,validation_data=(test_X,test_y),verbose=2,shuffle=False)
	...
	##############################
	# make 24 hour predictions here
	onevalue=yhat[0]
	newpredictions=[]
	newpredictions.append(onevalue)
	#newpredictions2=test_X
	#newpredictions2[0]=onevalue
	for j in range(24):
        #onevalue=yhat[0]
		newarray=model.predict(onevalue)
		onevalue=newarray[0]
		#newpredictions2[0]=onevalue
		newpredictions.append(onevalue)
		#print("####")
	...
	print("new 24 predictions")
	#for j in newpredictions:
	inv_newpredictions=scaler.inverse_transform(newpredictions)
	inv_newpredictions=inv_newpredictions[:,0]

    ##################################
	test_X=test_X.reshape((test_X.shape[0],n_hours*n_features))
	#invert sclaing for forecats
	print('test_X shape')
	print(test_X.shape)
	inv_yhat=concatenate((yhat,test_X[:,-33:]),axis=1)
	inv_yhat=scaler.inverse_transform(inv_yhat)
	inv_yhat=inv_yhat[:,0]
	#invert sclaing for actula data
	...
	pyplot.plot(inv_newpredictions, label='new_predicted')
	...
	responses=requests.get(`https://my.es.net/graphql?${querystring.stringify(q)}`)
	...
	new_values=[20557096903,29057277654,16540980355,22055456426,16305231588,18141175320]
