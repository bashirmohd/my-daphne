...
path_to_data = 'https://raw.githubusercontent.com/esnet/daphne/master/Multivariate-TimeSeries/One_year_data_sarima/L27.csv?token=AKKJU5QZOVJQ5FJP73II3VK5VM34Q'
...
plt.title('Bandwidth  Over Time In Hours')
...
plt.rcParams["font.weight"] = "bold"
plt.rcParams["axes.labelweight"] = "bold"
plt.title('Yearly Bandwidth Utilization (Gbps)')
...
pred_dynamic = results.get_prediction(start=pd.to_datetime('2018-11-01'), dynamic=True, full_results=True)
...
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime('2018-11-01'), average_daily_speed.index[-1],
                 alpha=.1, zorder=-1)
...
pred_uc.predicted_mean.plot(ax=ax, label='24-hr prediction')
ax.fill_between(pred_ci.index,
                pred_ci.iloc[:, 0],
...
y_truth = average_daily_speed['2018-11-01':]
