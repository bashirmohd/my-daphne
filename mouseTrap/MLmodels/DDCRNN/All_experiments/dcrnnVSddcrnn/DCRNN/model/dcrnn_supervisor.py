...
class DCRNNSupervisor(object):
    ...
    def run_epoch_generator(self, sess, model, data_generator, return_output=False, training=False, writer=None):
        ...
        for idx, (x, y) in enumerate(data_generator):
            ...
            '''
            if idx < 20:
                filename1 = os.path.join(self._data_kwargs['dataset_dir'] + '/detailed_results/dcrnn_input_%s.h5' %str(idx))               
                filename2 = os.path.join(self._data_kwargs['dataset_dir'] + '/detailed_results/dcrnn_prediction_%s.h5' %str(idx))               
                filename3 = os.path.join(self._data_kwargs['dataset_dir'] + '/detailed_results/dcrnn_output_%s.h5' %str(idx))               

                df_input = pd.DataFrame(x[0, :, :, 0])
                df_pred = pd.DataFrame(vals['outputs'][0, :, :, 0])
                df_label = pd.DataFrame(y[0, :, :, 0])

                df_input.to_hdf(filename1, 'results')
                df_pred.to_hdf(filename2, 'results')
                df_label.to_hdf(filename3, 'results')
            '''
