...
class DCGRUCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or "dcgru_cell"):
            with tf.variable_scope("gates"):  # Reset gate and update gate.
                ...
                #value = tf.nn.tanh(fn(inputs, state, output_size, bias_start=1.0))
                #value = tf.reshape(value, (-1, self._num_nodes, output_size))
                #inp, st = tf.split(value=value, num_or_size_splits=2, axis=-1)
                #inp = tf.reshape(inp, (-1, self._num_nodes * self._num_units))
                #st = tf.reshape(st, (-1, self._num_nodes * self._num_units))

            # RNN
            #output = fn(new_state, output_size, bias_start=1.0)
            #print('output', output.shape)
            #output = new_state = inp + st
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        #print('inputs_and_state', inputs_and_state.shape)
    ...
    def _gconv(self, inputs, output_size, bias_start=0.0):
        ...
        inputs = tf.reshape(inputs, (batch_size, self._num_nodes, -1))
        #state = tf.reshape(state, (batch_size, self._num_nodes, -1))
        inputs_and_state = inputs #tf.concat([inputs, state], axis=2)
        input_size = inputs_and_state.get_shape()[2].value
        dtype = inputs.dtype
