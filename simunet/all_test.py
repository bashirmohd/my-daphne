	...
	for j in range(TRAININGITERATION):
		...
		for i in range(EPISODE_NUMBER): #doesnot do anything now.. depends on flows
			...
			for fa in all_flows:
				...
				###########Set memory to ZERO
				#alloted_eorm_matrix[0][0]=0
				#alloted_eorm_matrix[0][1]=0
				#alloted_eorm_matrix[1][0]=0
				#alloted_eorm_matrix[1][1]=0
				#alloted_eorm_matrix[2][0]=0
				#alloted_eorm_matrix[2][1]=0
				#alloted_eorm_matrix[3][0]=0
				#alloted_eorm_matrix[3][1]=0
...
def test(learned_q_table, got_test_flows):
...
def random_test(got_test_flows):
	...
	for rfd in rec_flows_done:
		print(rfd)
	print("End of RANDOM game")
			


def elephant_first(got_test_flows):
	#Create agents
	...
	for fa in got_test_flows:
		if fa.name=="elephant":
			only_elephant.append(fa)
		if fa.name=="mouse":
			only_mice.append(fa)
	for se in only_elephant:
		sort_elephant_first.append(se)
	for sm in only_mice:
		sort_elephant_first.append(sm)


	fanum=0
	for fa in sort_elephant_first:
		#get current utilization matrix
		...
		#path_chosen = random.randint(0,3)
		#take first flow
		if fanum==0 or fanum==9 or fanum==19 or fanum==29 or fanum==39 or fanum==49 or fanum==59 or fanum==69 or fanum==79 or fanum==89 or fanum==99:
			print_throughput0=10-avail_util_paths[0]
			print_throughput1=10-avail_util_paths[1]
			print_throughput2=8-avail_util_paths[2]
			print_throughput3=8-avail_util_paths[3]
...
def mice_first(got_test_flows):
	...
	for fa in got_test_flows:
		...
		if fa.name=="mouse":
			only_mice.append(fa)
	for sm in only_mice:
		sort_mice_first.append(sm)

	for se in only_elephant:
		sort_mice_first.append(se)
	
	fanum=0

	for fa in sort_mice_first:
		#get current utilization matrix
		fi=fa # flow to allocate first one
...
def main():
	...
	#generate elephant or mice flow 100 flows
	for f in range(Flows_no):
		EorM=random.randint(1,10)
		#if between 2-9 mouse else elephant
		if(EorM>=3):
			flowname="mouse"
			flowsize=random.randint(1,2)
		...
		#add active flows to queue
		flow_created=FlowTraffic(f, flowname, flowsize,flowdur, 0,0)
		test_flows.append(flow_created)

	print("flows created")
	print(len(test_flows))
	...
	test(learned_q_table, test_flows)
	print("size of qtable %s" %len(learned_q_table))

	random_test(test_flows)

	elephant_first(test_flows)

	mice_first(test_flows)
