...
np.random.seed(1)
UNIT = 30 #changes the size of tetris 
HEIGHT = 20  
WIDTH = 8 
MID = (WIDTH / 2 - 1) * UNIT #8 divided by 1 * 30 = 240
dy = [UNIT, 0, 0]  #30,0,0
dx = [0, UNIT, -UNIT] #0,30,-30
PLUS_SCORE = 10.0
...
class Env(tk.Tk):
    ...
    def possible_to_move(self, action):
        for n in range(len(self.block)):
            ...
            #makes sure not to go outside borders
           # 1 =d , 2= l, r=3
    ...
    def move(self, action):
        ...
        if flag == 2:
            for n in range(4): # saving last 4 q states
                s = (self.canvas.coords(self.block[n]))
                y = int(s[1] / UNIT)
                x = int(s[0] / UNIT)
                self.score += self.score_weight[y]
                ret += 0 #self.score_weight[y]
                self.map[y][x] = 1
                print("1: score updated:", self.score)

            break_cnt = 0
            while True:
                y = self.is_map_horizon()
                print("map horizontal? ", y)
                if y == -1:
                    break
            ...
            print("2: score updated:", self.score)
